...
func InitServiceLogger(cfg config.ConfigProvider) (*zap.Logger, error) {
	conf := cfg.GetAppConfig()
	logLevel, err := mapLogLevel(conf.Logger.LogLevel)
	if err != nil {
		return nil, fmt.Errorf("failed to set log level: %w", err)
	}
	// Create the logger based on environment and log level
	logger, err := NewLogger(logLevel, conf.App.Env)
	if err != nil {
		return nil, err
	}

	// Add the service name as a field for every log entry
	return logger.With(zap.String("service", conf.App.Name)), nil
}
